...
model = MagNet_link_prediction(q=args.q, K=args.K, num_features=2, hidden=16, label_dim=2).to(device)
...
for split in list(link_data.keys()):
    ...
    for epoch in range(args.epochs):
        train_loss, train_acc = train(X_real, X_img, y, edge_index, edge_weight, query_edges)
        val_acc = test(X_real, X_img, y_val, edge_index, edge_weight, query_val_edges)
    ...
    test_acc = test(X_real, X_img, y_test, edge_index, edge_weight, query_test_edges)
