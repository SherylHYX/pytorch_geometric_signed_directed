...
from torch_geometric_signed_directed.utils import link_class_split, in_out_degree, directed_features_in_out
...
for split in list(link_data.keys()):
    ...
    for epoch in range(args.epochs):
        train_loss, train_acc = train(
            X, y, edge_index, edge_in, in_weight, edge_out, out_weight, query_edges)
        val_acc = test(X, y_val, edge_index, edge_in, in_weight,
                       edge_out, out_weight, query_val_edges)
    ...
    test_acc = test(X, y_test, edge_index, edge_in, in_weight,
                    edge_out, out_weight, query_test_edges)
