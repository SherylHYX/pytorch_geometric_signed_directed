...
from torch_geometric_signed_directed.utils import directed_link_class_split, in_out_degree, get_appr_directed_adj
from torch_geometric_signed_directed.nn.directed import DiGCN_link_prediction
...
def train(X, y, edge_index, edge_weight, query_edges):
    model.train()
...
def test(X, y, edge_index, edge_weight, query_edges):
    model.eval()
    with torch.no_grad():
        out = model(X, edge_index=edge_index, 
                    query_edges=query_edges, 
...
model = DiGCN_link_prediction(num_features=2, hidden=16, label_dim=2).to(device)
...
for split in list(link_data.keys()):
    ...
    edge_index, edge_weight = get_appr_directed_adj(args.alpha, edge_index, data.x.shape[0], 
    ...
    for epoch in range(args.epochs):
        train_loss, train_acc = train(X, y, edge_index, edge_weight, query_edges)
        val_acc = test(X, y_val, edge_index, edge_weight, query_val_edges)
    ...
    test_acc = test(X, y_test, edge_index, edge_weight, query_test_edges)
