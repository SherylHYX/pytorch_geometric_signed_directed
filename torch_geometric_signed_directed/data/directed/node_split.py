...
def node_class_split(data:torch_geometric.data.Data, 
                ...
                seed:npt.NDArray[np.int_]=[], data_split:int=10) -> torch_geometric.data.Data:
    ...
    Args:
        ...
        seed (IntArray or IntList with the length of data_split): The random seed list for each data split.
...
def sample_per_class(random_state:np.random.RandomState, labels:npt.NDArray[np.int_], num_examples_per_class:int, 
                        forbidden_indices:npt.NDArray[np.int_]=None):
    ...
    Args:
        ...
        labels (npt.NDArray[np.int_]): Node labels array.
        num_examples_per_class (int): Number of nodes per class. 
        forbidden_indices (npt.NDArray[np.int_]): Nodes to be avoided when selection.
    """
...
def get_train_val_test_split(random_state:np.random.RandomState,
                             ...
                             train_examples_per_class:int=None, val_examples_per_class:int=None,
                             test_examples_per_class:int=None,
                             train_size:int=None, val_size:int=None, 
                             test_size:int=None) -> Union[npt.NDArray[np.int_],npt.NDArray[np.int_],npt.NDArray[np.int_]]:
    r"""This function is obtained from https://github.com/flyingtango/DiGCN/blob/main/code/Citation.py
    ...
    Args:
        ...
        train_size_per_class (int, optional): The size per class of random splits for the training dataset.  
        val_size_per_class (int, optional): The size per class of random splits for the validation dataset.
        test_examples_per_class (int, optional): The size per class of random splits for the testing dataset. 
                    (Default: None. All nodes not selected for training/validation are used for testing)
    """
    num_samples = labels.shape[0]
    num_classes = labels.max()+1
