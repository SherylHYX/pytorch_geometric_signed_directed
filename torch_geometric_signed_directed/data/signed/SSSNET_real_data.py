...
class SSSNET_real_data(InMemoryDataset):
    ...
    def _generate_url(self, name: str) -> Tuple:
        if name.lower() == 'sampson':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/Sampson')
        elif name.lower() == 'wikirfa':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/wikirfa')
        elif self.name[:8].lower() == 'fin_ynet':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/Fin_YNet')
        elif name.lower() == 'sp1500':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/SP1500')
        elif name.lower() == 'ppi':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/PPI')
        elif name.lower() == 'rainfall':
            url = ('https://github.com/SherylHYX/pytorch_geometric_signed_directed/raw/main/datasets/rainfall')
        return url
    ...
    def process(self):
        ...
        if self.name.lower() == 'sampson':
            data = Data(num_nodes=indices.max().item() + 1, edge_index=indices, edge_weight=values, x=features, y=labels)
        elif self.name.lower() in ['sp1500', 'rainfall'] or self.name[:8].lower() == 'fin_ynet':
            data = Data(num_nodes=indices.max().item() + 1, edge_index=indices, edge_weight=values, y=torch.LongTensor(labels))
        else:
