...
class SGCNConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], pos_edge_index: Adj,
        ...
        if self.first_aggr:
            ...
            out_n = self.propagate(neg_edge_index, x=x)
            out_n = self.lin_u(torch.cat([out_n, x[0]], dim=-1))

            return torch.cat([out_b, out_n], dim=-1)

        else:
            F_in = self.in_channels
            out_b1 = self.propagate(pos_edge_index, x=(x[0][..., :F_in], x[1][..., :F_in]), stop=True)
            out_b2 = self.propagate(neg_edge_index, x=(x[0][..., F_in:], x[1][..., F_in:]))
            out_b = torch.cat([out_b1, out_b2, x[0][..., :F_in]], dim=-1)
            out_pos = self.lin_b(out_b)

            out_n1 = self.propagate(pos_edge_index, x=(x[0][..., F_in:], x[1][..., F_in:]))
            out_n2 = self.propagate(neg_edge_index, x=(x[0][..., :F_in], x[1][..., :F_in]))
            out_n = torch.cat([out_n1, out_n2, x[0][..., F_in:]], dim=-1)
            out_n = self.lin_u(out_n)

            return torch.cat([out_pos, out_n], dim=-1)
