...
class SiGAT(nn.Module):
    ...
    Args:
        ...
        device (torch.device): Device for the model. Default to 'cpu'.
    """
    def __init__(self, node_num: int, edge_index_s,
                 in_emb_dim: int=20, hidden_emb_dim: int=20, batch_size=500, device='cpu'):
        super().__init__()
        ...
        self.mlp_layer = nn.Sequential(
            nn.Linear(hidden_emb_dim * (len(self.adj_lists) + 1), hidden_emb_dim),
            nn.Tanh(),
    ...
    def forward(self, nodes):
        ...
        for edges, agg in zip(self.edge_lists, self.aggs):
            ...
            nodes_1, edges_1, inv, _ = k_hop_subgraph(nodes_t, 1, edges, num_nodes=self.node_num, flow='target_to_source', relabel_nodes=True)
